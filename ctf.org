...
+ Can be stealthy and communicate over periodical HTTP connections
  * Can also run interactively, but that creates more noise
+ Has a team server
+ Beacons can proxy other beacons for less noise on the perimeter
  * For example the domain controller may not have internet connection
  * Can use named pipes / SMB to talk to each other
  * SMB chaining
+ Different ways to deliver
+ can add targets manually / from nmap outputs
+ scanning
+ ticket functionality
  * steal live kerberos tickets
  * create tickets using compromised accounts credentials
  * this is very difficult to detect (pass the ticket)
+ Tunneling
  * SOCKS proxy
  * Covert VPN
+ SLEEP Scripting language
+ Establishing persistence is key
+ demo:
  * can generate a payload, host it and supply a one-liner to download it
  * can generate various versions - macro, stageless, etc
...
*Looks like we could get bob's credentials*

I should migrate to a 64-bit process
- see.exe is shown as x86
...
bob seems to be a domain admin
Domain admins should not be allowed to log in to admins
...
-> I could be using an old version of mimikatz
...
+ Alternative way to set up route:
  ...
  * set SESSION 1
  * set SUBNET 10.59.3.0
  * run
  * NOTE: needed to change the command from ~add auto~ to ~add~
